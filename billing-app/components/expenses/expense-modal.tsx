...
export function ExpenseModal({
  ...
  const [cgstPercent, setCgstPercent] = useState(initialData?.cgstPercent?.toString() || '')
  const [sgstPercent, setSgstPercent] = useState(initialData?.sgstPercent?.toString() || '')
  const [igstPercent, setIgstPercent] = useState(initialData?.igstPercent?.toString() || '')
    ...
    try {
      ...
      if (isKarnataka) {
        if (cgstPercent === '' || sgstPercent === '') {
          throw new Error('CGST and SGST percentages are required for Karnataka')
        }
      } else {
        if (igstPercent === '') {
      ...
      if (isNaN(amountNum) || amountNum <= 0) throw new Error('Please enter a valid amount greater than zero')
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg bg-white border-slate-200 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-2 overflow-y-auto flex-1 pr-2">
          {/* Bill / Expense Name */}
          ...
          <div className="space-y-2">
            ...
            <Select value={category} onValueChange={setCategory} disabled={isSaving} required>
          ...
          {/* State Dropdown - Full list of Indian states & UTs */}
          <div className="space-y-2">
            ...
            <Select value={state} onValueChange={setState} disabled={isSaving} required>
          ...
          {state && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {isKarnataka ? (
                <>
                  <div className="space-y-2">
                    <Label htmlFor="cgst" className="text-sm font-medium text-slate-700 flex items-center gap-2">
                    ...
                    <Input
                      ...
                      onWheel={(e) => e.currentTarget.blur()} // ← Disable mouse wheel / swipe increment
                      required
                      disabled={isSaving}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="sgst" className="text-sm font-medium text-slate-700 flex items-center gap-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="igst" className="text-sm font-medium text-slate-700 flex items-center gap-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onWheel={(e) => e.currentTarget.blur()} // ← Disable swipe/increment
              required
              disabled={isSaving}
